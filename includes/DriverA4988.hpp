...
class a4988 {
  public:
    // constructors:
    a4988(int motor_steps, int ms1_pin, int ms2_pin, int ms3_pin, int dir_pin, int enable_pin, int step_pin, bool direction);
    ...
    int dir_pin;
    int enable_pin;
        // step counting inside function calls (reset to zero after func is canceled or finished)
    int StepCounter = 0;
    // count global steps since the start of programm: usefull when limit are checked 
    int TotalStepCounted=0;
};
